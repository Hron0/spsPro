...
export function SearchBar({ onSearch, initialSearch = "", initialStartDate, initialEndDate }: FilterBarProps) {
    ...
    }, [initialSearch, initialStartDate, initialEndDate]) // Removed onSearch dependency
        ...
        <div className="w-full max-w-3xl space-y-2">
            <div className="relative w-full">
                ...
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setIsFiltersOpen(!isFiltersOpen)}>
                        <CalendarIcon className="h-4 w-4" />
                    </Button>
                    {hasActiveFilters && (
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={handleClearFilters}>
                            <X className="h-4 w-4" />
                        </Button>
                    )}
                </div>
            ...
            {isFiltersOpen && (
                <div className="flex flex-col sm:flex-row gap-2 p-2 border rounded-md bg-white">
